...
* make a small contribution to a repository through a pull request on GitHub.
...
In this lesson, you will learn how to start contributing to open source. The upcoming lessons will walk you through not only the technical elements associated with contributing to open source but also the social elements that are good to understand. 

## 1. Get to know the GitHub repository. 

The first step in contributing to open source is finding and then getting to know the GitHub repository where the code is stored. 

In this case, you will practice contributing to a sample pyOpenSci repository where you are set up to be successful and can practice without worrying about making mistakes. Making mistakes is actually the best way to learn. 
...
To begin, you should get to know the repository that you wish to contribute to, better. To do this:
...
1. Read through the documentation. Most people's first contribution is to documentation. This is a great place to start because typos and other issues often make for great first pull requests, and you can often make the changes fully in the GitHub interface online versus needing to clone the repository and work locally.
...
1. Read through the open issues in the repository. Reading through open issues can be a great way to gage what the project needs help with. 

**If you already have a fix in mind that you want to make:**

If you want to fix something you found, such as a typo in the documentation, you first want to see if there is an open issue you could work on directly. If an open issue about the topic doesn't exist, you should create a new GitHub issue that details the problem that you found and wish to fix.
...
When scanning the issues, look for ones tagged "good first issue". or "beginner friendly". Not all maintainers will tag issues but if you see such tags, then you know you are in a repository that welcomes new contributions. 
...
1. If there is already an issue open for the item that you want to work on, you can leave a comment indicating your interest in working on it. That comment might look something like this:
...
2. Be **specific about what you'd like to fix** in the issue. If it's a bug that you are fixing in the code, provide a **fully reproducible code example** of how to trigger the bug so the maintainers can easily understand the problem. If it is a documentation fix, then link to the documentation page that has the error and be specific about what you'd like to fix, add, or enhance. 

:::{todo}
Link to the stravalib issue that I recently worked on where the reproducible example was missing.
:::

The content of your issue is more important than you might think. Maintainers are often volunteers, working on projects in their free time. The more information you can provide to them, the easier it is for them to understand what your goal is and how to support you. Be as specific as you can be in your issue! This might mean that it takes you some time to create the issue. The time invested up front will pay off in the issue moving forward.

3. Be patient. Once you have opened your issue, be patient, as it could take some time for maintainers to respond. The timeline will vary based on how active the repository issue is, how many open issues there are, the size of the maintainer team, and even what's going on in the maintainer's life at that particular time.

It could take a few weeks for someone to get back to you. 

4. If the maintainers invite you to submit a pull request addressing the issue that you left or commented on, you're good to go. At that point, it's time to
...
1. go to the xxx repository on GitHub
2. Exxplore the documentation and code. YOu will find many items that could be fixed, including spelling errors, typos, and more.
3. Pick something that you would like to fix, and open an issue that specifies what you'd like to implement.
...
:::{note}
For this activity, focus on the process of contributing and making a small change to a single file, You don't need to make a huge change or fix everything in a file. Most first contributions are small updates to documentation. Start there, build confidence. You can always submit more issues and pull requests later!  
:::
